...
static esp_err_t touch_gt911_i2c_read(esp_lcd_touch_handle_t tp, uint16_t reg, uint8_t *data, uint8_t len)
{
    ...
    uint16_t cmd = reg;
#if CT_TOUCH_REG_ADDR_SWAP
    cmd = (uint16_t)((reg << 8) | (reg >> 8));
#endif

    /* Read data */
    return esp_lcd_panel_io_rx_param(tp->io, cmd, data, len);
}
...
static esp_err_t touch_gt911_i2c_write(esp_lcd_touch_handle_t tp, uint16_t reg, uint8_t data)
{
    ...
    // *INDENT-OFF*
    /* Write data */
    return esp_lcd_panel_io_tx_param(tp->io, cmd, (uint8_t[]){data}, 1);
    // *INDENT-ON*
}
